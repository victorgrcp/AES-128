...
class AES:
    ...
    def __init__(self, m: str, key: str, inputHex=False, keyHex=False):
        ...
        self.key    = np.array(key) if keyHex else np.array([hex(ord(c)) for c in key], dtype='<U4')
        self.state  = np.array(m) if inputHex else np.array([hex(ord(c)) for c in m], dtype='<U4')
        
    def expand_key(self):
        
        # Primera iteración usando la key original
        self.keys[0, 0]    = hex(int(self.key[0], 0)  ^ self.sub_byte(self.key[7]) ^ self.r_con[0])
        self.keys[0, 4]    = hex(int(self.key[4], 0)  ^ self.sub_byte(self.key[11]) ^ 0)
        self.keys[0, 8]    = hex(int(self.key[8], 0)  ^ self.sub_byte(self.key[15]) ^ 0)
        self.keys[0, 12]   = hex(int(self.key[12], 0) ^ self.sub_byte(self.key[3]) ^ 0)
        for i in range(1, 4):
        ...
        for nround in range(1, 10):
            ...
            self.keys[nround, 4]    = hex(int(self.keys[nround-1, 4], 0)  ^ self.sub_byte(self.keys[nround-1, 11]) ^ 0)
            self.keys[nround, 8]    = hex(int(self.keys[nround-1, 8], 0)  ^ self.sub_byte(self.keys[nround-1, 15]) ^ 0)
            self.keys[nround, 12]   = hex(int(self.keys[nround-1, 12], 0) ^ self.sub_byte(self.keys[nround-1, 3]) ^ 0)
            for i in range(1, 4):
